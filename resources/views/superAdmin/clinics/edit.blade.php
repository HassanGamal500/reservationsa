...
@section('content')
    ...
    <div class="container-fluid">
        ...
        @endif
        <form method="post" action="{{route('update_clinic', $clinics[0]->id)}}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input type="tel" pattern="^[0-9]*$" name="clinic_phone" class="form-control" maxlength="13" placeholder="{{trans('admin.enter phone')}}" value="{{$clinics[0]->phone}}">
            </div>
            <div class="form-group">
                <label>{{trans('admin.upload photo')}}</label>
                <input type="file" name="clinic_image" class="form-control-file">
                <br>
                <input type="hidden" name="old_image" value="{{$clinics[0]->image}}">
                <img src="{{asset($clinics[0]->image)}}" class="img-thumbnail" height="70px" width="100px">
            </div>
            <div class="form-group">
                <label>{{trans('admin.description part')}} ({{trans('admin.english')}})</label>
                <input type="text" pattern="^[A-Za-z0-9_.,/{}@#!~%()-<>\s]+$" name="clinic_description_part[1]" class="form-control" maxlength="140" placeholder="{{trans('admin.enter description part')}}" value="{{$clinics[0]->description_part}}">
            </div>
            <div class="form-group">
                <label>{{trans('admin.description part')}} ({{trans('admin.arabic')}})</label>
                <input type="text" pattern="^[\u0621-\u064A\u0660-\u0669\u06f0-\u06f9\u0600-\u06FF\sA-Za-z0-9_.,/{}@#!~%()<>-]+$" name="clinic_description_part[2]" class="form-control" maxlength="140" placeholder="{{trans('admin.enter description part')}}" value="{{$clinics[1]->description_part}}">
            </div>
            <div class="form-group">
                <label>{{trans('admin.description full')}} ({{trans('admin.english')}})</label>
                <textarea type="text" pattern="^[A-Za-z0-9_.,/{}@#!~%()-<>\s]+$" name="clinic_description_full[1]" class="form-control" placeholder="{{trans('admin.enter description full')}}">{{$clinics[0]->description_full}}</textarea>
            </div>
            <div class="form-group">
                <label>{{trans('admin.description full')}} ({{trans('admin.arabic')}})</label>
                <textarea type="text" pattern="^[\u0621-\u064A\u0660-\u0669\u06f0-\u06f9\u0600-\u06FF\sA-Za-z0-9_.,/{}@#!~%()<>-]+$" name="clinic_description_full[2]" class="form-control" placeholder="{{trans('admin.enter description full')}}">{{$clinics[1]->description_full}}</textarea>
            </div>
            <div class="form-group">
                ...
                <select class="form-control country_id" name="country_id">
                    ...
                    <option value="{{$country->id}}" {{$clinics[0]->country_id == $country->id ? "selected" : ""}}>{{$country->name}}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                ...
                <select class="form-control city_id" name="city_id" id="">
                    ...
                    <option value="{{$city->id}}" {{$clinics[0]->city_id == $city->id ? "selected" : ""}}>{{$city->name}}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                <label>{{trans('admin.latitude')}}</label>
                <input type="text" name="clinic_latitude" class="form-control" placeholder="{{trans('admin.enter latitude')}}" value="{{$clinics[0]->latitude}}">
            </div>
            <div class="form-group">
                <label>{{trans('admin.longitude')}}</label>
                <input type="text" name="clinic_longitude" class="form-control" placeholder="{{trans('admin.enter longitude')}}" value="{{$clinics[0]->longitude}}">
            </div>
            <hr>
            <div class="form-group">
